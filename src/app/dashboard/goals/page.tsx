...
interface GoalForm {
  name: string; targetAmount: string;
  savedAmount: string; currency: string; color: string;
}
...
export default function GoalsPage() {
  ...
  // "goal-form" modal: null=closed, "add"=new, Goal object=editing
  ...
  function openEdit(g: Goal) {
    setForm({
      ...
      savedAmount: g.savedAmount, currency: g.currency, color: g.color,
    });
    setFormModal(g);
  }

  function openFunds(g: Goal) { setFundsModal(g); setAddAmount(""); }
  ...
  return (
    <div className="space-y-5">
      ...
      {loading ? (
        <div className="text-center py-16 text-sm text-gray-300">Loading goals…</div>
      ) : goals.length === 0 ? (
                    ...
                    <Card key={goal.id}>
                      ...
                      <div className="flex items-center justify-between">
                        <span className="text-xs text-gray-400">
                          {pct.toFixed(0)}% · {formatCurrency(target - saved, goal.currency)} to go
                        </span>
                        <button
                          onClick={() => openFunds(goal)}
      ...
      {/* Add funds modal */}
      <Modal open={fundsModal !== null} onClose={() => setFundsModal(null)} title={`Add funds — ${fundsModal?.name}`}>
        <div className="space-y-4">
