...
interface BudgetLimit {
  id: string; category: string; limitAmount: string; currency: string; month: number; year: number;
}
...
export default function BudgetsPage() {
  ...
  function limitToDisplay(amount: number, limitCurrency: string): string {
    if (limitCurrency === displayCurrency) return formatCurrency(amount, displayCurrency);
    const amountInUsd = amount / (rates[limitCurrency] ?? 1);
    return formatCurrency(amountInUsd * (rates[displayCurrency] ?? 1), displayCurrency);
  }
  ...
  function openModal(cat: string) {
    ...
    setEditCat(cat);
    if (existing) { setLimitInput(existing.limitAmount); setCurrency(existing.currency); }
    else { setLimitInput(""); setCurrency(preferredCurrency); }
    setShowModal(true);
  }
  ...
  async function handleSave() {
    ...
    try {
      ...
      await fetchLimits(); setShowModal(false);
  ...
  async function handleDelete(limitId: string) {
    ...
    try {
      ...
      await fetchLimits();
    } catch (e) { alert("Could not delete budget."); console.error(e); }
      ...
      <div className="space-y-4">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-gray-400 dark:text-gray-500 mt-0.5">Set and track spending limits per category</p>
          </div>
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
            <div className="flex items-center gap-1 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl px-1 self-start sm:self-auto">
              ...
              <span className="text-sm font-semibold text-gray-800 dark:text-white px-2 min-w-[110px] text-center">
                {MONTHS[month-1]} {year}
              </span>
        ...
        {displayCategories.length === 0 ? (
          <Card className="flex flex-col items-center justify-center py-16 gap-3">
            ...
            <p className="text-gray-400 dark:text-gray-500 text-sm">No budgets or spending this month yet.</p>
            <Button size="sm" onClick={() => openModal(expenseCategories[0].name)}>Set your first budget</Button>
          </Card>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {displayCategories.map((cat, i) => {
              ...
              const capUsd   = limit ? Number(limit.limitAmount) / (rates[limit.currency] ?? 1) : null;
              ...
              return (
                ...
                >
                  <Card className={cn("h-full", isOver && "border-red-200 dark:border-red-900 bg-red-50/30 dark:bg-red-900/10", isWarn && "border-amber-200 dark:border-amber-900 bg-amber-50/30 dark:bg-amber-900/10")}>
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <div className="flex items-center gap-1 shrink-0">
                        ...
                        <button onClick={() => openModal(cat.name)}
                        ...
                        {limit && (
                          <button onClick={() => handleDelete(limit.id)} disabled={deleting === limit.id}
                    ...
                    <div className="flex items-baseline justify-between mb-2">
                      <span className={cn("text-xl font-bold", isOver ? "text-red-600 dark:text-red-400" : "text-gray-900 dark:text-white")}>
                        {toDisplay(spentUsd)}
                      </span>
                      {limit
                        ? <span className="text-xs text-gray-400 dark:text-gray-500">of {limitToDisplay(Number(limit.limitAmount), limit.currency)} limit</span>
                    ...
                    {capUsd !== null && pct !== null && (
                      <div className="h-2 bg-gray-100 dark:bg-gray-800 rounded-full overflow-hidden">
                        <motion.div
                          ...
                          className={cn("h-full rounded-full", isOver ? "bg-red-500" : isWarn ? "bg-amber-400" : "bg-green-500")}
                        />
                      </div>
                    )}
                    {isOver && capUsd !== null && <p className="text-xs text-red-500 dark:text-red-400 mt-2 font-medium">Over by {toDisplay(spentUsd - capUsd)}</p>}
                    {isWarn && <p className="text-xs text-amber-600 dark:text-amber-400 mt-2 font-medium">{(100 - pct!).toFixed(0)}% of budget remaining</p>}
                    {!isOver && !isWarn && capUsd !== null && pct !== null && (
                      <p className="text-xs text-gray-400 dark:text-gray-500 mt-2">{(100 - pct).toFixed(0)}% remaining</p>
                    )}
                  </Card>
                </motion.div>
              );
        ...
        <Modal open={showModal} onClose={() => setShowModal(false)} title="Set budget limit">
          <div className="space-y-4">
            <Select label="Category" value={editCat} onChange={e => {
              const cat = e.target.value; setEditCat(cat);
              const existing = limits.find(l => l.category === cat);
              if (existing) { setLimitInput(existing.limitAmount); setCurrency(existing.currency); }
              else { setLimitInput(""); setCurrency(preferredCurrency); }
            }} options={expenseCategories.map(c => ({ value: c.name, label: `${c.icon} ${c.name}` }))} />
            <div className="grid grid-cols-2 gap-3">
              <Input label="Limit amount" type="number" min="1" step="0.01" placeholder="300.00"
            ...
            <p className="text-xs text-gray-400 dark:text-gray-500">Warning shows at 80% of limit.</p>
