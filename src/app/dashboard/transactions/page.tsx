      ...
      <div className="space-y-4">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-gray-400 mt-0.5">{transactions.length} transaction{transactions.length !== 1 ? "s" : ""}</p>
          </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white border border-gray-100 rounded-2xl px-4 py-3">
          ...
          <div className="flex items-center gap-4 sm:gap-6 text-sm">
            <div><span className="text-gray-400 text-xs block">Income</span><span className="font-semibold text-green-600">+{formatCurrency(totalIncome)}</span></div>
            <div><span className="text-gray-400 text-xs block">Expenses</span><span className="font-semibold text-red-500">-{formatCurrency(totalExpense)}</span></div>
            <div><span className="text-gray-400 text-xs block">Balance</span>
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
            ...
            <div className="flex gap-2">
              ...
              {(search || category || type) && (
                <button onClick={() => { setSearch(""); setCategory(""); setType(""); }}
                  className="text-xs text-gray-400 hover:text-gray-600 px-2 whitespace-nowrap">Clear</button>
              )}
            </div>
          </div>
        </Card>
        ...
        <Card className="p-0 overflow-hidden">
          {loading ? (
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
              ...
              <Button size="sm" variant="secondary" onClick={() => setModal("add")}>Add your first one</Button>
            </motion.div>
          ) : (
            <AnimatePresence>
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex flex-wrap items-center gap-1.5 mt-0.5">
                        ...
                        {tx.note && <span className="text-xs text-gray-400 truncate max-w-[140px] hidden sm:inline">· {tx.note}</span>}
                      </div>
                    </div>
